...
const studentFields = { ...baseFields, cgpa: { type: Number, min: 0, max: 10, default: null } };
...
// Uses Google Gemini (via @google/generative-ai) to score a student's resume and
// store the parsed JSON result into the student's document.
const {TextDecoder} = require('util');
let genaiClient = null;
const rawGemKey = process.env.GEMINI_API_KEY;
if (!rawGemKey) {
  console.warn('GEMINI_API_KEY not set in environment. Generative AI calls will be disabled.');
} else {
  const apiKey = String(rawGemKey).replace(/^['"]|['"]$/g, '');
  try {
    // Try ResponsesClient first
    const ga = require('@google/generative-ai');
    const ResponsesClient = ga.ResponsesClient || ga.TextServiceClient || ga.TextGenerationServiceClient || ga.default?.ResponsesClient;
    if (ResponsesClient) {
      try {
        genaiClient = new ResponsesClient({ apiKey });
        console.log('Generative AI client initialized using ResponsesClient');
      } catch (e) {
        console.warn('ResponsesClient instantiation failed:', e && e.message ? e.message : e);
      }
    }

    // Fallback: if ResponsesClient not available or failed, try top-level GoogleGenerativeAI
    if (!genaiClient && ga.GoogleGenerativeAI) {
      try {
        genaiClient = new ga.GoogleGenerativeAI(apiKey);
        console.log('Generative AI client initialized using GoogleGenerativeAI');
      } catch (e) {
        console.warn('GoogleGenerativeAI instantiation failed:', e && e.message ? e.message : e);
        genaiClient = null;
      }
    }

    if (!genaiClient) {
      console.warn('@google/generative-ai installed but no compatible client could be initialized.');
    }
  } catch (e) {
    console.warn('Failed to initialize @google/generative-ai client:', e && e.message ? e.message : e);
    genaiClient = null;
  }
}
...
app.post('/api/score-resume/:id', async (req, res) => {
  ...
  try {
    ...
    const base64 = student.resumeData;
    if (!base64) return res.status(400).json({ message: 'No resume data found for student' });

    if (!genaiClient) return res.status(500).json({ message: 'Generative AI client not configured on server' });
    ...
    try {
      ...
      return res.status(400).json({ message: 'Invalid base64 resume data' });
    }

    // Try to extract text using Tesseract first (as requested), fall back to pdf-parse
    let ocrText = '';
    ...
    try {
      if (createTesseractWorker) {
        ...
        // tesseract.js accepts Buffers for images; for PDFs this may fail depending on input.
        try {
          const { data } = await worker.recognize(pdfBuffer);
          ocrText = data && data.text ? data.text : '';
        } catch (ocrErr) {
          console.warn('tesseract recognize failed, will fallback to pdf-parse:', ocrErr && ocrErr.message ? ocrErr.message : ocrErr);
        }
        try { await worker.terminate(); } catch(e){}
      }
    } catch (e) {
      console.warn('Tesseract OCR attempt failed:', e && e.message ? e.message : e);
    }

    // Fallback to pdf-parse if OCR returned little/no text
    try {
      if ((!ocrText || ocrText.trim().length < 50) && pdfParse) {
        const data = await pdfParse(pdfBuffer);
        if (data && data.text && data.text.trim().length > (ocrText ? ocrText.length : 0)) {
          ocrText = data.text;
        }
      }
    } catch (e) {
      console.warn('pdf-parse failed:', e && e.message ? e.message : e);
    }

    // Save extracted text into the student doc for debugging/search
    try {
      await Student.findByIdAndUpdate(id, { resumeText: ocrText }, { new: false });
    } catch (e) {
      console.warn('Failed to save resumeText to student doc:', e && e.message ? e.message : e);
    }

  // Prepare request to Gemini: prefer model from .env, normalize to SDK expected form
  // Note: exact client API may differ; normalize GEMINI_MODEL into a model path if needed
  const modelEnv = process.env.GEMINI_MODEL || 'gemini-1.5-flash';
  const model = modelEnv.startsWith('models/') ? modelEnv : `models/${modelEnv}`;

    // The @google/generative-ai client may accept input in different shapes; attempt a generic call
    const request = {
      model,
      // Provide an instruction system prompt to parse the resume and return JSON
      // We'll include the inline PDF content as bytes in 'input' using "inline" style if supported.
      // Since client shapes can vary, attempt a simple 'text' request with a preamble instructing
      // the model to read the provided base64 PDF attached in the 'files' array.
      input: `You are a resume parser. Parse the attached PDF (provided as base64). Return a JSON object with fields: ats_metrics (total_score, breakdown), suggested_roles (array), extracted_skills (array), projects (array). Respond with only valid JSON.`,
      // Attach inline data according to the client's supported structure
      // We'll include a 'files' field with name, mime, and data
      files: [
        {
          name: 'resume.pdf',
          mime: 'application/pdf',
          // some clients expect raw bytes; we will pass base64 and hope the client accepts it
          data: base64,
        },
      ],
    };
    ...
    let genResponseText = null;
    try {
      // Attempt several client method shapes depending on SDK version
      let genResp = null;
      if (genaiClient.generateText) {
        const [resp] = await genaiClient.generateText({ model: model, text: request.input, ...(request.files ? { files: request.files } : {}), temperature: 0.2 });
        genResp = resp;
      } else if (genaiClient.generate) {
        const resp = await genaiClient.generate({ model, input: request.input, files: request.files, temperature: 0.2 });
        genResp = resp;
      } else if (genaiClient.create) {
        // e.g., ResponsesClient.create
        const resp = await genaiClient.create({ model, input: request.input, files: request.files, temperature: 0.2 });
        genResp = resp;
      } else if (genaiClient.getGenerativeModel) {
        // Fallback to GoogleGenerativeAI usage: getGenerativeModel(...).generateContent(...)
        // The SDK expects content entries shaped as { role: 'user'|'system', parts: [ { text: '...' } ] }
        const modelKey = process.env.GEMINI_MODEL || 'gemini-1.5-flash';
        const modelObj = genaiClient.getGenerativeModel({ model: modelKey });
        // Prefer OCR-extracted text if available, otherwise fall back to the plain input instruction
        const contentText = (ocrText && ocrText.trim().length > 0) ? ocrText : request.input;
        const resp = await modelObj.generateContent({
          systemInstruction: request.input,
          contents: [ { role: 'user', parts: [ { text: contentText } ] } ],
        });
        genResp = resp;
      } else {
        throw new Error('Generative client does not expose supported call methods (generateText/generate/create/getGenerativeModel)');
      }

      // Try common locations for text output
      if (!genResp) throw new Error('Empty response from generative API');

      // normalize
      if (genResp.candidates && genResp.candidates[0]) {
        genResponseText = genResp.candidates[0].output || genResp.candidates[0].content || genResp.candidates[0].text || null;
      }
      if (!genResponseText && genResp.output) {
        if (Array.isArray(genResp.output)) genResponseText = genResp.output.map(o=>o.content||o.text||'').join('\n');
        else genResponseText = genResp.output.content || genResp.output.text || null;
      }
      if (!genResponseText && genResp.result) genResponseText = genResp.result;
      if (!genResponseText && genResp.text) genResponseText = genResp.text;

      if (Array.isArray(genResponseText)) genResponseText = genResponseText.join('\n');
    } catch (e) {
      console.error('Generative API call failed - full error:', e);
      // include stack or message for debugging
      const errMsg = e && (e.message || e.toString()) ? (e.message || e.toString()) : 'Unknown error';
      return res.status(500).json({ message: 'Generative API call failed', error: errMsg, details: e });
    }

    // Parse JSON safely
    const parsed = safeParseJson(genResponseText);
    if (!parsed) return res.status(500).json({ message: 'Failed to parse JSON from generative API', raw: genResponseText });

    // Update the student document with the returned fields
    const update = {};
    if (parsed.ats_metrics !== undefined) update.ats_metrics = parsed.ats_metrics;
    if (parsed.suggested_roles !== undefined) update.suggested_roles = parsed.suggested_roles;
    if (parsed.extracted_skills !== undefined) update.extracted_skills = parsed.extracted_skills;
    if (parsed.projects !== undefined) update.projects = parsed.projects;

    const updated = await Student.findByIdAndUpdate(id, update, { new: true }).lean();
    return res.json({ message: 'ATS scoring saved', result: parsed, student: updated });
  } catch (err) {
    console.error('Score resume error:', err && err.message ? err.message : err);
    return res.status(500).json({ message: 'Internal error', error: err && err.message ? err.message : String(err) });
  }
});

// Diagnostic endpoint to test Gemini/Generative AI connectivity and client method shapes
app.get('/api/gemini-test', async (req, res) => {
  if (!process.env.GEMINI_API_KEY) return res.status(400).json({ message: 'GEMINI_API_KEY not set in .env' });
  if (!genaiClient) return res.status(500).json({ message: 'Generative AI client not initialized on server' });
  const modelName = process.env.GEMINI_MODEL || 'gemini-1.5-flash';
  try {
    // Try multiple invocation styles and report which one succeeded
    // 1) If genaiClient has getGenerativeModel (GoogleGenerativeAI)
    if (genaiClient.getGenerativeModel) {
      try {
        const modelObj = genaiClient.getGenerativeModel({ model: modelName });
        const resp = await modelObj.generateContent({ contents: [{ role: 'user', parts: [{ text: 'Hello from test: please return {"ok": true}' }] }] });
        return res.json({ method: 'getGenerativeModel.generateContent', ok: true, response: resp });
      } catch (e) {
        // continue to next
        console.warn('getGenerativeModel.generateContent failed:', e && e.message ? e.message : e);
      }
    }

    // 2) If genaiClient.generateText exists
    if (genaiClient.generateText) {
      try {
        const [resp] = await genaiClient.generateText({ model: modelName, text: 'Hello from test: please return {"ok": true}' });
        return res.json({ method: 'generateText', ok: true, response: resp });
      } catch (e) {
        console.warn('generateText failed:', e && e.message ? e.message : e);
      }
    }

    // 3) If genaiClient.generate exists
    if (genaiClient.generate) {
      try {
        const resp = await genaiClient.generate({ model: modelName, input: 'Hello from test: please return {"ok": true}' });
        return res.json({ method: 'generate', ok: true, response: resp });
      } catch (e) {
        console.warn('generate failed:', e && e.message ? e.message : e);
      }
    }

    // 4) If genaiClient.create exists (ResponsesClient style)
    if (genaiClient.create) {
      try {
        const resp = await genaiClient.create({ model: modelName, input: 'Hello from test: please return {"ok": true}' });
        return res.json({ method: 'create', ok: true, response: resp });
      } catch (e) {
        console.warn('create failed:', e && e.message ? e.message : e);
      }
    }

    return res.status(500).json({ message: 'No supported client method succeeded. Check server logs for details.' });
  } catch (err) {
    console.error('Gemini test unexpected error:', err);
    return res.status(500).json({ message: 'Unexpected error', error: err && err.message ? err.message : String(err), details: err });
  }
});
  ...
  app.post(`/api/${pathName}/update`, async (req, res) => {
    try {
      ...
      const allowedFieldsByRole = {
        ...
        tpo: ['name', 'phone', 'employeeId', 'profileData', 'profileFileName'],
